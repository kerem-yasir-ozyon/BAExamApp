...
public class QuestionController : AdminBaseController
{
    ...
    public async Task<IActionResult> GetQuestionsByGivenValues(AdminQuestionListVM adminQuestionListVM, State state)
    {
        ...
        if (selectedSubjectItem == null)
        {
            subtopicList = await GetSubtopic();
            selectedSubtopicItem = subtopicList.FirstOrDefault(x => x.Value == adminQuestionListVM.SubtopicName.FirstOrDefault() || x.Text == adminQuestionListVM.SubtopicName.FirstOrDefault());
        ...
        {
            subtopicList = await GetSubtopic(selectedSubjectItem.Value);
            selectedSubtopicItem = subtopicList.FirstOrDefault(x => x.Value == adminQuestionListVM.SubtopicName.FirstOrDefault() || x.Text == adminQuestionListVM.SubtopicName.FirstOrDefault());
            if (selectedSubtopicItem != null)
            {
                selectedSubtopicItem.Selected = true;
    ...
    public async Task<IActionResult> QuestionCreate(AdminQuestionCreateVM adminQuestionCreateVM, IFormCollection collection)
    {
        ...
        ViewBag.QuestionTypes = GetQuestionTypes();
        ViewBag.SubjectList = await GetSubjects();
        ViewBag.SubtopicList = await GetSubtopics(adminQuestionCreateVM.SubjectId);
